...
class FxRateService:
    """
    Service for looking up USD/CAD exchange rates from Bank of Canada.
    
    This is the main entry point for all exchange rate operations.
    It uses the Bank of Canada Valet API to fetch real-time and historical rates.
    """
    
    def __init__(self, client: Optional[BankOfCanadaClient] = None):
        """
        Initialize the FX Rate Service.
        
        Args:
            client: Optional BankOfCanadaClient instance. If not provided,
    ...
    def get_rate_for_date(
        ...
        observation = self._client.get_rate_for_date(
    ...
    def get_monthly_rates(
        ...
        observations = self._client.get_rates_for_year(direction.series_name, year)
    ...
    def close(self):
        """Close the underlying client if owned by this service."""
